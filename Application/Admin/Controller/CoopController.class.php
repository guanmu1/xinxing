...
class CoopController extends CommonController {
    public function index(){
        ...
        $count = M('Coop')->alias('n')->where($where)->count();

        $Page  = new \Common\Util\Page($count,'Coop/index',$size);// 实例化分页类
        $list  =  M('Coop')->alias('n')
            ->field('n.coop_id,n.coop_title,n.coop_link,n.create_time,l.lang_name,n.group_id')
            ->join("left join trade_lang l ON l.lang_code = n.language")
            ->where($where)
            ->group('n.group_id')
            ->order('l.lang_level asc,n.coop_id desc')
    ...
    public function add(){
        if (IS_POST) {
            ...
            foreach ($formData as $key => $value) {
                $oneData = array();
                $oneData['coop_title'] = $value['title'];
                $oneData['coop_link'] = $value['img'];
            ...
            if (M('Coop')->addAll($insertArr)) {
                $this->success('发布成功',U('Coop/index'));
            } else {
                $this->error('发布失败');
            }
        } else {
    ...
    public function edit(){
        if (IS_POST) {
            ...
            if (M('Coop')->addAll($insertArr)) {
                $this->success('修改成功',U('Coop/index'));
            } else {
                $this->error('修改失败');
            }
        } else {
            ...
            $allInfo = M('Coop')->where('group_id=%d and deleted=0',$groupId)->select();
            ...
            for ($i = 0; $i < 7; $i++) {
                ...
                foreach ($allInfo as $value) {
                    if ($value['form_id'] == $i) {
                        $rInfo[$i]['coop_title'] = $value['coop_title'];
                        $rInfo[$i]['coop_link'] = $value['coop_link'];
    ...
    public function revoke(){
        $videoId = I('get.coop_id');
        $videoData['video_status'] = 0;
        M('Coop')->where('coop_id=%d',$videoId)->save($videoData);
        $this->success('撤销成功',U('Coop/index'));
    }
    ...
    public function del(){
        ...
        $videoData['flag'] = 0;
        M('Coop')->where('coop_id=%d',$videoId)->save($videoData);
        $this->success('删除成功',U('Coop/index'));
    }
