    ...
    <div id="templatemo_main_content">
    	<div id="templatemo_sidebar">
            <div class="sidebar_content_box" style="height:100px;">
                <h3 style="text-align:center;margin-top:20px;color:red;font-weight:bold;"><?php echo L('业务产品');?></h3>
                <h5 style="text-align:center;margin-top:10px;color:#35bf35;">MEDIA CENTER</h5>
            </div>
            <div class="sidebar_content_box" style="background:#fff;padding:0;">
                <ul  style="padding:0;">
                    <li class="mouseover-li left-sub-menu-hover" style="margin: 10px auto;"><a class="left-sub-menu-a-hover" href="<?php echo U('Range/index');?>" ><?php echo L('业务范围');?></a></li>
                    <li style="height: 1px;background-color: #9FA0A1;"></li>
                    <li class="mouseover-li left-sub-menu" style="margin-bottom: 10px;margin-top: 10px;"><a class="left-sub-menu-a" href="<?php echo U('Goods/index');?>" ><?php echo L('产品展示');?></a></li>
                    <!--<li class="second-li"><a class="" href="#" ><?php echo L('衣帽');?></a></li>-->
                    <!--<li class="second-li"><a class="" href="#" ><?php echo L('鞋靴');?></a></li>-->
                    ...
                    <?php if(is_array($category)): $i = 0; $__LIST__ = $category;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cItem): $mod = ($i % 2 );++$i;?><li class="second-li"><a class="" href="<?php echo U('Goods/index',array('cpid'=>$cItem['category_id']));?>" ><?php echo L($cItem['category_name']);?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
                </ul>
            </div>
        ...
        <div id="templatemo_content">
            ...
            <div class="news_container" style="position: relative;overflow: hidden;">
                ...
                <div style="margin-top: 20px;margin-bottom: 20px;" id="show_imageflow_name">
                    <div style="height: 2px;background:#9FA0A1;float: left;width: 130px;margin-top: 14px;"></div>
                    <img onclick="prevImg()" style="float: left;margin-left: 20px;height: 30px;cursor: pointer;" src="/xinxing/Public/images/zuo.png" alt="">

                    <div style="background: #D7D7D8;width:200px;text-align: center;margin: 0 auto;font-size: 16px;line-height: 30px;float: left;margin-left: 40px;">
                        <div style="width: 10px;height: 30px;background: red;float: left;"></div>
                        <span class="top-show-name"><?php echo ($info[0]['range_title']); ?></span>
                        <div style="width: 10px;height: 30px;background: red;float: right;"></div>
                    </div>
                    <img onclick="nextImg()" style="float: left;margin-left: 40px;height: 30px;cursor: pointer;" src="/xinxing/Public/images/you.png" alt="">

                    <div style="height: 2px;background:#9FA0A1;float: right;width: 130px;margin-top: 14px;"></div>
                    <div style="clear: both;"></div>
                </div>
                
                <div class="area_img">
                    <?php if(is_array($info)): foreach($info as $key=>$item): ?><img src="<?php echo ($item["range_img"]); ?>" width="700" height="360" title="<?php echo ($item["range_title"]); ?>" <?php if($key != 0): ?>style="display:none;"<?php endif; ?>><?php endforeach; endif; ?>
                </div>

                <p style="text-align: center;margin-top: 20px;font-size: 16px;color: gray;">中国新兴进出口总公司业务范围，遍及全世界100多个国家</p>
...
<script>
    var cShow = 0;
    function prevImg(){
        var len = $('.area_img img').length;
        $('.area_img img').hide();
        cShow = cShow <= 0 ? len-1 : cShow-1;
        $('.area_img img').eq(cShow).show();
        $('.top-show-name').text($('.area_img img').eq(cShow).attr('title'));
    }

    function nextImg(){
        var len = $('.area_img img').length;
        $('.area_img img').hide();
        cShow = cShow >= len-1 ? 0 : cShow+1;
//        alert(cShow + "__" + len);
        $('.area_img img').eq(cShow).show();
        $('.top-show-name').text($('.area_img img').eq(cShow).attr('title'));
    }
