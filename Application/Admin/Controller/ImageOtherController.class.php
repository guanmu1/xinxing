...
class ImageOtherController extends CommonController {
    public function index(){
        $size = I('get.size') ? I('get.size') : 20;
        $where['n.deleted'] = 0;

        $count =M('ImageOther')->alias('n')
            ->join("left join trade_lang l ON l.lang_code = n.language")
            ->where('n.deleted=0')
            ->count('distinct n.group_id');

        $Page  = new \Common\Util\Page($count,'ImageOther/index',$size);// 实例化分页类
        $list  =  M('ImageOther')->alias('n')
    ...
    public function add(){
        if (IS_POST) {
            ...
            if (M('ImageOther')->addAll($insertArr)) {
                $this->success('发布成功',U('ImageOther/index'));
            } else {
                $this->error('发布失败');
            }
        } else {
    ...
    public function edit(){
        if (IS_POST) {
            ...
            M('ImageOther')->where('group_id = %d',$groupId)->save(array('deleted'=>1));

            M('ImageOther')->where('group_id = %d',$groupId)->delete();
            ...
            if (M('ImageOther')->addAll($insertArr)) {
                $this->success('修改成功',U('ImageOther/index'));
            } else {
                $this->error('修改失败');
            }
        } else {
            ...
            $allInfo = M('ImageOther')->where('group_id=%d and deleted=0',$groupId)->select();
    ...
    public function revoke(){
        ...
        M('ImageOther')->where('image_id=%d',$videoId)->save($videoData);
        $this->success('撤销成功',U('ImageOther/index'));
    }
    ...
    public function del(){
        ...
        $re = M('ImageOther')->where('group_id=%d',$groupId)->save(array('deleted'=>1));
        $returnResult = array();
        if ($re) {
